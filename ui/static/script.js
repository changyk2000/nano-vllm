    ...
    const loadAnalytics = async ({ silent = false } = {}) => {
        ...
        try {
            ...
            if (!hasLatency && !hasDiff && !hasSparsity && !hasTimeline) {
                ...
                analyticsEmpty.textContent = 'No analytics data available. Please run queries to generate analytics.';
            } else {
                analyticsEmpty.style.display = 'none';
            }

            if (!silent) {
                setStatus(analyticsStatus, 'Analytics refreshed successfully.');
            }
        } catch (error) {
            analyticsEmpty.style.display = 'block';
            analyticsEmpty.textContent = error.message || 'Unable to fetch analytics data.';
